...
const FP_State = {
  data: null,
  load() {
    const raw = localStorage.getItem(FP_CONFIG.storageKey);
    if (raw) {
  ...
  save() {
    ...
    try {
      localStorage.setItem(FP_CONFIG.storageKey, JSON.stringify(this.data));
  ...
  reset() { localStorage.removeItem(FP_CONFIG.storageKey); this.init(); }
};
...
const FP_Utils = {
  ...
  getSelectionDeadline(festivalDate, logistics) {
    const mode = FP_CONFIG.logisticsModes[logistics];
  ...
  getMilestoneDates(festival) {
    const logistics = FP_State.getFestival(festival.id).logistics;
    const mode = FP_CONFIG.logisticsModes[logistics];
    return FP_CONFIG.milestones.map(ms => {
  ...
  getUrgency(festival) {
    const festState = FP_State.getFestival(festival.id);
    const fDate = this.parseDate(festival.date);
    const today = this.today();
    if (fDate < today) return "past";
    const deadline = this.getSelectionDeadline(festival.date, festFP_State.logistics);
    const days = this.diffDays(deadline, today);
    if (days < 0) {
      if (festFP_State.milestones.selection) return "plan";
      return "urgent";
...
const FP_Render = {
  monthNav() {
    document.getElementById("monthNav").innerHTML =
      FP_CONFIG.months.map(m => `<a href="#month-${m.num}">${m.label}</a>`).join("");
  },

  dashboard() {
    const today = FP_Utils.today();
    const stats = { urgent: 0, week: 0, month: 0, plan: 0, past: 0 };
    FESTIVALS.forEach(f => stats[FP_Utils.getUrgency(f)]++);

    // 最近需备货节点
    const upcoming = FESTIVALS
      .filter(f => FP_Utils.getUrgency(f) !== "past")
      .map(f => ({ fest: f, deadline: FP_Utils.getSelectionDeadline(f.date, FP_State.getFestival(f.id).logistics) }))
      .sort((a, b) => a.deadline - b.deadline)[0];

    const cd = document.getElementById("countdown");
    if (upcoming) {
      const days = FP_Utils.diffDays(upcoming.deadline, today);
      const u = FP_Utils.getUrgency(upcoming.fest);
      cd.innerHTML = `今日 <strong>${FP_Utils.fmtYMD(today)}</strong> · 最近备货节点：
        <strong>${upcoming.fest.icon} ${upcoming.fest.name}</strong>（${upcoming.fest.date}）
        · 选品截止 <strong>${FP_Utils.fmtYMD(upcoming.deadline)}</strong>
        · <span class="badge ${u}">${FP_Utils.urgencyLabel(u)}</span>`;
    }
    ...
    document.getElementById("statCards").innerHTML = cards.map(c =>
      `<div class="stat-card ${c.cls} ${FP_Filter.statCardUrgency === c.key ? "active" : ""}" onclick="FP_Filter.statCardUrgency='${c.key}';FP_Filter.search='';FP_Filter.urgency='${c.key}';document.getElementById('filterSearch').value='';document.getElementById('filterUrgency').value='${c.key}';FP_Interact.applyFilter();">
  ...
  main() {
    const filtered = FESTIVALS.filter(f => {
      const festState = FP_State.getFestival(f.id);
      const urgency = FP_Utils.getUrgency(f);
      if (FP_Filter.category && !f.products.some(p => p.category === FP_Filter.category)) return false;
      if (FP_Filter.month && f.month !== parseInt(FP_Filter.month)) return false;
      const effU = FP_Filter.statCardUrgency || FP_Filter.urgency;
      if (effU && urgency !== effU) return false;
      if (FP_Filter.status && festFP_State.status !== FP_Filter.status) return false;
      if (FP_Filter.search) {
        const q = FP_Filter.search.toLowerCase();
  ...
  festivalCard(f) {
    const festState = FP_State.getFestival(f.id);
    const urgency = FP_Utils.getUrgency(f);
    const deadline = FP_Utils.getSelectionDeadline(f.date, festFP_State.logistics);
    const days = FP_Utils.diffDays(deadline, FP_Utils.today());
    const expanded = f.importance === "S" ? "expanded" : "";
    const catLabel = ({ festival:"节日", activity:"活动", trend:"趋势" })[f.category] || f.category;

    const header = `
      <div class="card-header" tabindex="0" role="button" aria-expanded="${expanded ? "true" : "false"}" onclick="FP_Interact.toggleCard('${f.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();FP_Interact.toggleCard('${f.id}')}">
        <div>
          <div class="card-title">
            ...
            <span class="badge ${urgency}">${FP_Utils.urgencyLabel(urgency)}</span>
          </div>
          <div class="card-meta">
            ${f.date} · ${catLabel} | 选品截止 ${FP_Utils.fmtYMD(deadline)}（${days >= 0 ? "剩" + days + "天" : "已过" + (-days) + "天"}）
          </div>
        </div>
    ...
    const body = `<div class="card-body">
      ${this.logisticsToggle(f.id, festFP_State.logistics)}
      ${this.timeline(f)}
      ${this.products(f)}
      ${this.validation(f)}
      ${this.controls(f)}
  ...
  logisticsToggle(id, logistics) {
    const btns = LOGISTICS_OPTIONS.map(o =>
      `<button class="${o.id === logistics ? "active" : ""}"
        onclick="FP_Interact.switchLogistics('${id}','${o.id}')">${o.icon} ${o.label}</button>`
    ).join("");
  ...
  timeline(f) {
    const festState = FP_State.getFestival(f.id);
    const ms = FP_Utils.getMilestoneDates(f);
    const nodes = ms.map(m => {
      const done = festFP_State.milestones[m.id];
      return `<div class="timeline-node">
        <div class="timeline-dot ${done ? "done" : ""}"
          tabindex="0" role="button"
          aria-label="${m.name} ${m.dateStr}${done ? '（已完成）' : '（未完成）'}"
          onclick="FP_Interact.toggleMilestone('${f.id}','${m.id}')"
          onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();FP_Interact.toggleMilestone('${f.id}','${m.id}')}"
          title="${m.actions}"></div>
        <div class="ms-name">${m.name}</div>
        <div class="ms-date">${m.dateStr}</div>
      </div>`;
    }).join("");
    const mode = FP_CONFIG.logisticsModes[festFP_State.logistics];
  ...
  products(f) {
    ...
    const tabs = `<div class="product-cat-tabs">
      <div class="product-cat-tab active" onclick="FP_Interact.filterProductCat('${f.id}','',this)">${f.products.length} 全部</div>
      ${cats.filter(c => f.products.some(p => p.category === c)).map(c => {
        const cnt = f.products.filter(p => p.category === c).length;
        return `<div class="product-cat-tab" onclick="FP_Interact.filterProductCat('${f.id}','${c}',this)">${FP_CONFIG.categories[c].label} ${cnt}</div>`;
      }).join("")}
    </div>`;
    const rows = f.products.map(p => {
      const sel = festFP_State.selectedSkus.includes(p.sku);
      const riskCls = p.riskLevel === "低" ? "risk-low" : p.riskLevel === "中" ? "risk-mid" : "risk-high";
      return `<tr class="prod-row" data-cat="${p.category}">
        <td><input type="checkbox" ${sel ? "checked" : ""} onchange="FP_Interact.toggleSku('${f.id}','${FP_Utils.escape(p.sku)}')" title="标记要做"></td>
        ...
        <td><span class="stars">${FP_Utils.stars(p.matchScore)}</span></td>
        <td><span class="risk ${riskCls}" title="${FP_Utils.escape(p.riskNote)}">${p.riskLevel}</span></td>
        <td class="kw-cell">${p.keywords.map(kw => `<a class="amazon-kw" href="https://www.amazon.co.uk/s?k=${encodeURIComponent(kw)}" target="_blank" rel="noopener" title="在亚马逊UK搜索: ${FP_Utils.escape(kw)}"><span class="amz-icon">🔍</span>${FP_Utils.escape(kw)}</a>`).join("")}</td>
      </tr>`;
    }).join("");
  ...
  validation(f) {
    ...
    const trendLinks = (v.googleTrends || []).map(kw =>
      `<a class="amazon-kw" href="https://www.amazon.co.uk/s?k=${encodeURIComponent(kw)}" target="_blank" rel="noopener" title="在亚马逊UK搜索: ${FP_Utils.escape(kw)}"><span class="amz-icon">🔍</span>${FP_Utils.escape(kw)}</a>`
    ).join(" ");
    return `<div class="validation-section">
      <h4>🔍 验证指引（数据依据）</h4>
      <ul>
        <li><strong>Google Trends：</strong>${trendLinks}</li>
        <li><strong>亚马逊验证：</strong>${FP_Utils.escape(v.amazonCheck || "")}</li>
        <li><strong>1688拿货：</strong>${FP_Utils.escape(v.sourcing || "")}</li>
      </ul>
      ${v.riskFlags && v.riskFlags.length ? `<div style="margin-top:6px;color:var(--urgent)"><strong>⚠️ 风险提示：</strong>${v.riskFlags.map(r => FP_Utils.escape(r)).join("；")}</div>` : ""}
    </div>`;
  },
  ...
  controls(f) {
    ...
    return `<div class="card-controls" style="margin-top:8px">
      <label>状态：</label>
      <select onchange="FP_Interact.setStatus('${f.id}',this.value)">
        ${opts.map(o => `<option value="${o.v}" ${festFP_State.status === o.v ? "selected" : ""}>${o.l}</option>`).join("")}
      </select>
    </div>
    <div>
      ...
      <textarea class="notes-area" placeholder="如：A店计划上架8款，B店5款；供应商：义乌XX"
        oninput="FP_Interact.setNotes('${f.id}',this.value)">${FP_Utils.escape(festFP_State.notes)}</textarea>
    </div>`;
  }
};
...
const FP_Interact = {
  ...
  switchLogistics(id, logistics) {
    FP_State.updateFestival(id, { logistics });
    // 只更新当前卡片内容，不重绘整个页面，避免卡片关闭
    const card = document.getElementById("card-" + id);
    if (card) {
      const wasExpanded = card.classList.contains("expanded");
      const festState = FP_State.getFestival(id);
      const f = FESTIVALS.find(f => f.id === id);
      if (f) {
        card.outerHTML = FP_Render.festivalCard(f);
  ...
  toggleSku(id, sku) { FP_State.toggleSku(id, sku); },
  setStatus(id, status) { FP_State.updateFestival(id, { status }); FP_Render.dashboard(); },
  setNotes(id, notes) { FP_State.updateFestival(id, { notes }); },
...
function bindFilters() {
  document.getElementById("filterCategory").addEventListener("change", e => {
    FP_Filter.category = e.target.value; FP_Filter.statCardUrgency = ""; FP_Render.main();
  });
  document.getElementById("filterMonth").addEventListener("change", e => {
    FP_Filter.month = e.target.value; FP_Render.main();
  });
  document.getElementById("filterUrgency").addEventListener("change", e => {
    FP_Filter.urgency = e.target.value; FP_Filter.statCardUrgency = ""; FP_Render.main();
  });
  document.getElementById("filterStatus").addEventListener("change", e => {
    FP_Filter.status = e.target.value; FP_Render.main();
  });
  const debouncedSearch = FP_Utils.debounce(v => { FP_Filter.search = v; FP_Render.main(); }, 180);
  document.getElementById("filterSearch").addEventListener("input", e => {
    debouncedSearch(e.target.value);
  });
  document.getElementById("resetFilter").addEventListener("click", () => FP_Interact.resetFilter());
}
...
function exportData() {
  const blob = new Blob([FP_State.export()], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `uk-festival-backup-${FP_Utils.fmtYMD(new Date())}.json`;
  a.click();
...
function importData(event) {
  ...
  reader.onload = e => {
    try {
      FP_State.import(e.target.result);
      alert("✅ 导入成功！");
...
function exportExcel() {
  ...
  FESTIVALS.forEach(f => {
    const urgency = FP_Utils.getUrgency(f);
    const festState = FP_State.getFestival(f.id);
    const urgencyLabel = FP_Utils.urgencyLabel(urgency);
    const statusLabel = ({none:"未启动",selection:"选品中",ordered:"已下单",arrived:"已到仓",listed:"已上架"})[festFP_State.status] || festFP_State.status;
...
function fpSaveSync() {
  ...
  document.getElementById('fpSyncStatus').textContent = '鉁?閰嶇疆宸蹭繚瀛?;
}
function fpLoadSync() {
  const c = fpGetSync();
...
async function fpSyncCreate() {
  const t = document.getElementById('fpToken').value.trim();
  if (!t) { document.getElementById('fpSyncStatus').textContent = '鉂?璇峰厛濉玊oken'; return; }
  try {
    ...
    fpSaveSync();
    document.getElementById('fpSyncStatus').textContent = '鉁?Gist鍒涘缓鎴愬姛锛両D: '+d.id;
  } catch(e) { document.getElementById('fpSyncStatus').textContent = '鉂?'+e.message; }
}
async function fpSyncPush() {
  const c = fpGetSync(); if (!c.token||!c.gistId) { document.getElementById('fpSyncStatus').textContent = '鉂?璇峰厛閰嶇疆'; return; }
  try {
    ...
    document.getElementById('fpSyncStatus').textContent = '鉁?鎺ㄩ€佹垚鍔燂紒'+new Date().toLocaleString('zh-CN');
  } catch(e) { document.getElementById('fpSyncStatus').textContent = '鉂?'+e.message; }
}
async function fpSyncPull() {
  ...
  try {
    ...
    FP_Render.dashboard(); FP_Render.main();
    document.getElementById('fpSyncStatus').textContent = '鉁?鎷夊彇鎴愬姛锛?+new Date().toLocaleString('zh-CN');
  } catch(e) { document.getElementById('fpSyncStatus').textContent = '鉂?'+e.message; }
}
function fpExport() {
...
function fpImport(e) {
  ...
  r.onload = ev => { try { FP_State.import(ev.target.result); FP_Render.dashboard(); FP_Render.main(); alert('鉁?瀵煎叆鎴愬姛锛?); } catch(err) { alert('鉂?'+err.message); } };
  r.readAsText(f); e.target.value = '';
}
function fpExportCSV() {
  const rows = [['鏈堜唤','鑺傛棩','鏃ユ湡','閲嶈鎬?,'SKU','鑻辨枃SKU','鍝佺被','鎴愭湰','鍞环','鍒╂鼎鐜?,'鍖归厤搴?,'椋庨櫓','椋庨櫓璇存槑','鍏抽敭璇?]];
  FESTIVALS.forEach(f => {
    const u = FP_Utils.getUrgency(f);
    const fs = FP_State.getFestival(f.id);
    f.products.forEach(p => {
      rows.push([f.month+'鏈?,f.name,f.date,f.importance,p.sku,p.skuEn,p.category,p.costRange,p.priceRange,p.margin,p.matchScore+'/5',p.riskLevel,p.riskNote,(p.keywords||[]).join('; ')]);
    });
  });
...
function fpReset() {
  if (!confirm('鈿狅笍 纭畾娓呴櫎鎵€鏈夎繘搴︼紵')) return;
